...
bool LarVio::initialize() {
  ...
  // debug log
  // std::cout << (output_dir+"msckf_2_state.txt").c_str() << std::endl;
  // fImuState.open("/home/touchair/output/larvio/msckf_state.txt");
  // fTakeOffStamp.open("/home/touchair/output/larvio/msckf_takeoff.txt");
  // fImuState.open((output_dir+"msckf_2_state.txt").c_str(), ios::out | ios::trunc | ios::app);
  // fTakeOffStamp.open((output_dir+"msckf_2_takeoff.txt").c_str(), ios::out | ios::trunc | ios::app);
...
bool LarVio::processFeatures(MonoCameraMeasurementPtr msg,
  ...
  // fImuState << std::fixed << std::setprecision(6) << px << " " << py << " " << pz << std::endl;
...
bool LarVio::processFeatures(MonoCameraMeasurementPtr msg,
        std::vector<ImuData>& imu_msg_buffer, std::vector<GpsData>& gps_msg_buffer)
{

  // std::cout << "gps msg buffer: " << gps_msg_buffer.size() << std::endl;

  if(!bFirstFeatures)
  {
    if((imu_msg_buffer.begin() != imu_msg_buffer.end()) && (imu_msg_buffer.begin()->timeStampToSec - msg->timeStampToSec - state_server.td <= 0.0))
    {
      bFirstFeatures = true;
    }
    else
  ...
  if(!is_gravity_set)
  {
    if(flexInitPtr->tryIncInit(imu_msg_buffer, msg, m_gyro_old, m_acc_old, state_server.imu_state))
    {
      is_gravity_set = true;
      take_off_stamp = state_server.imu_state.time;
      last_ZUPT_time = state_server.imu_state.time;
...
void LarVio::measurementUpdate_ZUPT_vpq () {
  ...
  // v在Legacy state中，p在滑动窗口中最新位置和次新位置
...
void LarVio::gpsCallback(std::vector<GpsData>& gps_msg_buffer)
{
  if(gps_msg_buffer.empty())
  {
    return;
  }
  ...
  GpsData msg = gps_msg_buffer[0];
  const int N = state_server.imu_states_augment.size();
  MatrixXd H = MatrixXd::Zero(3, state_server.state_cov.cols());

  H.block<3, 3>(0, 6) = Matrix3d::Identity();

  Vector3d res = Vector3d::Zero();

  Vector3d G_p_i = state_server.imu_state.position;
  Vector3d exp_G_p_Gps = gps_to_vio_r * point_enu;

  // std::cout << "G_p_i: " << G_p_i.transpose() << std::endl;
  // std::cout << "point_enu: " << point_enu.transpose() << std::endl;
  // std::cout << "exp_G_p_Gps: " << exp_G_p_Gps.transpose() << std::endl;

  // res = G_p_i - exp_G_p_Gps;
  // res = 观测值 - 状态值
  res = exp_G_p_Gps - G_p_i;
  res[2] = 0;

  
  // std::cout << "res: " << res.transpose() << std::endl;
  // std::cout << "res norm: " << res.norm() << std::endl;
  // printf("gps_msg_buffer size: %f\n", gps_msg_buffer.size());
  // std::cout << "gps msg buffer: " << gps_msg_buffer.size() << std::endl;
  ...
  // if(res.norm() > 10)
  // {
  //   // state_server.imu_state.position = exp_G_p_Gps;
  //   return;
  // }


  // Matrix3d R_GPS = 0.01 * gps_msg.cov;
  Matrix3d R_GPS = gps_msg.cov;

  MatrixXd& P = state_server.state_cov;
  MatrixXd S = H * P * H.transpose() + R_GPS;
  MatrixXd S_inv = S.llt().solve(Eigen::MatrixXd::Identity(S.rows(), S.cols()));
  MatrixXd K = P * H.transpose() * S_inv;
  // MatrixXd K_transpose = S.ldlt().solve(H * P);
  // MatrixXd K = K_transpose.transpose();

  VectorXd delta_x = K * res;

  // Update the legacy state
  const VectorXd& delta_x_legacy = delta_x.head(LEG_DIM);

  // std::cout << "delta_x_legacy size: " << delta_x_legacy.size() << std::endl;
  std::cout << "delta pose: " << delta_x_legacy.segment<3>(6).transpose() << std::endl;

  const Vector4d dq_imu = smallAngleQuaternion(delta_x_legacy.head<3>());
  Vector4d q_imu = state_server.imu_state.orientation;
  Quaterniond Q_imu = Quaterniond(dq_imu(3), dq_imu(0), dq_imu(1), dq_imu(2)) * Quaterniond(q_imu(3), q_imu(0), q_imu(1), q_imu(2));
  state_server.imu_state.orientation = Q_imu.coeffs();
  state_server.imu_state.velocity += delta_x_legacy.segment<3>(3);
  state_server.imu_state.position += delta_x_legacy.segment<3>(6);
  state_server.imu_state.gyro_bias += delta_x_legacy.segment<3>(9);
  ...
  // Update the IMU-CAM extrinsic
  const Vector4d dq_extrinsic = smallAngleQuaternion(delta_x_legacy.segment<3>(15));
  state_server.imu_state.R_imu_cam0 = state_server.imu_state.R_imu_cam0 * Quaterniond(dq_extrinsic(3), dq_extrinsic(0), dq_extrinsic(1), dq_extrinsic(2)).toRotationMatrix().transpose();
  state_server.imu_state.t_cam0_imu += delta_x_legacy.segment<3>(18);
  state_server.td += delta_x_legacy(21);
  ...
  // Update the augmented imu states and corresponding cam states
  auto imu_state_iter = state_server.imu_states_augment.begin();
  for(int i = 0; i < state_server.imu_states_augment.size(); ++i, ++imu_state_iter)
  {
    // update augmented imu state
    const VectorXd& delta_x_aug = delta_x.segment<6>(LEG_DIM+i*6);
    const Vector4d dq_IMU = smallAngleQuaternion(delta_x_aug.head<3>());
    ...
    const Vector4d& q_imu = imu_state_iter->second.orientation;
    Matrix3d R_b2w = Quaterniond(q_imu(3), q_imu(0), q_imu(1), q_imu(2)).toRotationMatrix();
    Matrix3d R_c2w = R_b2w * R_c2b;
    Quaterniond q_w_c = Quaterniond(R_c2w);
    imu_state_iter->second.orientation_cam = q_w_c.coeffs();
    imu_state_iter->second.position_cam = imu_state_iter->second.position + R_b2w * t_c_b;
  }
  ...
  for(int i = 0; i < state_server.feature_states.size(); ++i, feature_itr++)
  {
    FeatureIDType feature_id = (*feature_itr);
    IMUState_Aug imu_state_aug;
    if(use_schmidt && find(state_server.nui_ids.begin(), state_server.nui_ids.end(), map_server[feature_id].id_anchor) != state_server.nui_ids.end())
    {
      imu_state_aug = state_server.nui_imu_states[map_server[feature_id].id_anchor];
    }
    ...
    const Vector4d& cam_qua = imu_state_aug.orientation_cam;
    Matrix3d R_c2w = Quaterniond(cam_qua(3), cam_qua(0), cam_qua(1), cam_qua(2)).toRotationMatrix();
    const Vector3d& t_c_w = imu_state_aug.position_cam;
  ...
  if(use_schmidt && state_server.nui_ids.size() > 0)
  {
    MatrixXd P_nui = P.block(base_cntr+feature_idp_dim*state_server.feature_states.size(),
                             base_cntr+feature_idp_dim*state_server.feature_states.size(),
                             6*state_server.nui_ids.size(), 6*state_server.nui_ids.size());
